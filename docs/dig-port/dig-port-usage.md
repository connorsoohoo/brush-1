...
The `dino-extract` tool runs DINOv2 natively on Metal via [candle](https://github.com/huggingface/candle) (no Python needed), downloading weights from the Hugging Face hub (`facebook/dinov2-*`) on first run:

```bash
cargo run --release -p dino-extract -- --data /path/to/dataset
```

Constraints: supported models are `vits14`/`vitb14`/`vitl14` (no register variants or `vitg14`), and GPU acceleration is Metal/macOS only — on other platforms the tool falls back to CPU (minutes per image for ViT-B).
...
GARField-style hierarchical grouping, the interactive click-to-segment viewer, camera-pose optimization, and RSRD part tracking — see the "Out of scope" section of the plan.
